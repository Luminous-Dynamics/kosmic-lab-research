...
We introduce K-Index, a simple metric that predicts multi-agent coordination success by measuring behavioral flexibility—the negative correlation between observations and actions. Unlike entropy or mutual information, K-Index is computationally efficient (O(n) vs O(n²)), interpretable, and provides explicit boundary conditions. Across 30 experiments (N > 2,500 teams), we establish: (1) K-Index outperforms entropy-based metrics (r = 0.69 vs 0.17, p < 0.001), (2) flexibility regularization causally improves coordination by 6.9%, (3) the effect generalizes across algorithms (A2C: r = 0.88, REINFORCE: r = 0.71) and is robust under perturbation (r > 0.93), (4) the relationship reverses in sparse reward (r = -0.96) and zero-shot (r = -0.50) settings, revealing when flexibility becomes harmful. Early flexibility (episode 10) predicts final performance (r = 0.69), enabling early stopping. Power analysis confirms adequate statistical power (99.2% with n = 30). Our theoretical framework establishes that K-Index applies to discovery tasks requiring exploration with dense rewards and shared learning, but fails for execution tasks with obvious solutions. The metric's simplicity makes it practical for monitoring and improving real-world multi-agent systems.

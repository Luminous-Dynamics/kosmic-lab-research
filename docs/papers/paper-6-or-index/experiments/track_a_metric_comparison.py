#!/usr/bin/env python3
"""
Track A: Compare K-Index to Established Metrics

Compare K-Index (obs-action correlation) to:
1. Policy entropy
2. Action diversity (variance)
3. Mutual information proxy

Run with:
    nix-shell -p python3 python3Packages.numpy python3Packages.scipy --run "python3 -u track_a_metric_comparison.py"
"""
...
class Agent:
    ...
    def get_policy_entropy(self):
        ...
        # Use variance as entropy proxy (higher variance = higher entropy)
        return np.mean(np.var(actions, axis=0))

    def get_action_diversity(self):
        """Diversity: variance across time."""
        if len(self.action_history) < 10:
            return 0.0
        actions = np.array(self.action_history[-50:])
        # Total variance
    ...
    def get_mi_proxy(self):
        ...
        # Use correlation matrix as MI proxy
        # Higher absolute correlations = higher MI
        total_corr = 0
        n_pairs = 0
        for i in range(min(obs.shape[1], actions.shape[1])):
            if len(obs) > 1:
                c = np.corrcoef(obs[:, i], actions[:, i])[0, 1]
                if not np.isnan(c):
                    total_corr += abs(c)
                    n_pairs += 1
...
def main():
    print("\n" + "=" * 70)
    print("TRACK A: COMPARE K-INDEX TO ESTABLISHED METRICS")
    print("=" * 70)
    ...
    for i, m1 in enumerate(metric_names):
        for m2 in metric_names[i+1:]:
            r, p = stats.pearsonr(results[m1]['values'], results[m2]['values'])
    ...
    # Find best metric
    best = max(results.keys(), key=lambda k: abs(results[k]['r']))
    ...
    if best == 'kindex':
        ...
        print(f"\n→ {best} outperforms K-Index ({results[best]['r']:+.3f} vs {kindex_r:+.3f})")

    # Check if K-Index is competitive
    kindex_r = abs(results['kindex']['r'])
    others = [abs(results[k]['r']) for k in results if k != 'kindex']
    if kindex_r >= max(others) * 0.9:
        print("\n✓ K-Index is competitive with existing metrics")
        print("  Advantage: simpler and more interpretable")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"track_a_comparison_{timestamp}.npz"
